...
if os.path.exists(BUILD):
	if os.path.exists(OBJ_PREPROC):
		for path, dirs, files in os.walk(OBJ_PREPROC):
			for f in files:
				os.remove(os.path.join(path,f))
	if os.path.exists(PREPROC):
		for path, dirs, files in os.walk(PREPROC):
			for f in files:
				os.remove(os.path.join(path,f))
	if os.path.exists(OBJ_LZ77IMAGES) and BUNDLE_ASSETS:
		for path, dirs, files in os.walk(OBJ_LZ77IMAGES):
			for f in files:
				os.remove(os.path.join(path,f))
...
for dirname, dirnames, filenames in os.walk(LZ77IMAGES):
	for filename in filenames:
		if fnmatch.fnmatch(filename, '*.png'):
			...
			if not REBUILD_ASSETS and os.path.exists(os.path.join(dirname, image_pal)) and os.path.exists(os.path.join(dirname, image_bin)):
				continue
